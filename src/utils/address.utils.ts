...
export function validateAccountAddress(accountAddress: string, options?: { network?: NetworkT }): UtilValidationT {

    if (!accountAddress.includes('account_'))
        return invalidResult(errors.account.invalidAddress({ accountAddress }));

    try {
        validateRadixAddress({ address: accountAddress, network: options?.network });
    } catch (e) {
        return invalidResult(errors.account.invalidAddress({ accountAddress, verbose: e.message }));
    }
